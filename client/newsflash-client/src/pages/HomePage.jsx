...
export default function HomePage() {
  ...
  // Local controls (prefill q so it shows something on first load)
  ...
  }, [dispatch, status]); // one-time on mount for initial load
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end">
        <form onSubmit={onSearch} className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div>
            <label className="block text-xs text-[#113F67]/80 mb-1">From (YYYY-MM-DD)</label>
            <input
              value={dateFrom}
          ...
          <div>
            <label className="block text-xs text-[#113F67]/80 mb-1">To (YYYY-MM-DD)</label>
            <input
              value={dateTo}
      ...
      {status === "succeeded" && (
        <>
          <div className="mb-3 text-sm text-[#113F67]/80">
            Showing <span className="font-semibold text-[#113F67]">{showingRange}</span> of{" "}
            <span className="font-semibold text-[#113F67]">{totalResults}</span> results
          </div>
