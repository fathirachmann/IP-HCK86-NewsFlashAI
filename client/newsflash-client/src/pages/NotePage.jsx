...
export default function NotePage() {
  ...
  async function fetchArticle() {
    // jika sudah ada dari navigation state, pakai itu dulu
    ...
    <div className="mx-auto max-w-4xl px-4 py-6">
      ...
      <header className="mb-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-2xl font-semibold tracking-tight text-[#113F67]">Notes</h1>
            <div className="mt-1">
              <div className="text-sm text-[#113F67] font-medium truncate">{articleTitle}</div>
              {articleUrl && (
                <a
                  href={articleUrl}
                  target="_blank"
      ...
      {!loading && (
        <section className="rounded-2xl border border-[#113F67]/10 bg-white p-4 shadow-sm">
          <label className="block text-xs text-[#113F67]/80 mb-1">Add a note</label>
          <textarea
            value={newNote}
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm text-white ${
                saving ? "bg-[#58A0C8]/70 cursor-wait" : "bg-[#58A0C8] hover:opacity-90"
              }`}
            >
      ...
      {!loading && (
        <section className="mt-4 space-y-3">
          {notes.map((n) => {
            ...
            return (
              <div key={n.id} className="rounded-2xl border border-[#113F67]/10 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                ...
                {!isEditing ? (
                  <p className="mt-2 text-sm text-[#113F67] whitespace-pre-line">
                    {n.content || <span className="italic text-[#34699A]">Empty note</span>}
                  </p>
                ) : (
                  <textarea
                    value={editingText}
          ...
          {!notes.length && !err && (
            <div className="rounded-2xl border border-dashed border-[#113F67]/20 bg-white/70 p-8 text-center">
              ...
              <p className="text-sm text-[#34699A] mt-1">Tulis catatan pertama untuk artikel ini.</p>
            </div>
          )}
        </section>
      )}
